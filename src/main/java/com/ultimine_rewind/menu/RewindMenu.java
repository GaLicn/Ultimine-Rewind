...
public class RewindMenu extends AbstractContainerMenu {
    ...
    private final UltimineRecord record;
    ...
    public UltimineRecord record() {
        return record;
    }

    public List<MaterialRequirement> requiredMaterials() {
        return record != null ? record.requiredMaterials() : clientRequiredMaterials;
    }
    ...
    public boolean validateItems() {
        ...
        for (int index = 0; index < CONTAINER_SIZE; index++) {
            ...
            for (MaterialRequirement material : record.requiredMaterials()) {
                if (ItemStack.isSameItemSameTags(stack, material.stack())) {
                    return true;
                }
            }
        }
    ...
    public List<BlockRecord> restorableBlocks() {
        ...
        for (BlockRecord block : record.getBlocks()) {
            ItemStack required = block.getRequiredItem();
            int materialIndex = findMaterial(available, required);
            if (!required.isEmpty() && materialIndex >= 0
                    && available.get(materialIndex).count() >= 1) {
                MaterialRequirement material = available.get(materialIndex);
                available.set(materialIndex, new MaterialRequirement(material.stack(), material.count() - 1));
                result.add(block);
            }
        }
        return result;
    ...
    public void consumeFor(List<BlockRecord> restored) {
        ...
        if (!player.isCreative()) {
            for (BlockRecord block : restored) {
                addMaterial(consume, block.getRequiredItem(), 1);
            }
        }
